...
| Weekday preferences are a SOFT pull toward the ranked day, never a hard recurring assignment (2026-08-26) | Shivam's call. The 08-24 checkpoint recorded "each RA gets ONE recurring weekday" as confirmed; the solver never implemented it, and last year's real schedule shows the leads do not work that way either. Only 5 of 44 RAs had a single weekday; the median RA did 62% of their weekday shifts on their most-used day. A hard constraint would be less like their real schedule and could make a quarter infeasible. **This is not built. See AiCC task #4.** |
